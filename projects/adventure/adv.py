...
#________ USEFUL COMMANDS! ____________
# player.currentRoom.id
# player.currentRoom.getExits() 
# player.travel(direction)

# _________ Building Graph _____________
# graph = {}
# print(f'Number of Rooms: {len(roomGraph)}')

# # ________ Current Room & it's Exits ____
...
# print(player.travel(direction_to_travel))
...
# ___________ Building Graph __________________
graph = {}

# ______ Exit inverses & Traversal Path _______
exit_inverse = {'n':'s', 's':'n', 'e':'w', 'w':'e'}
traversalPath = []
roomPath = [] # to find nearest room with unexplored exits

# __________ Loading and Starting _____________
# world.loadGraph(roomGraph)
player = Player("Name", world.startingRoom)
...
# SAVE current room EXITS: 
current_exits = player.currentRoom.getExits()

# BUILD GRAPH for current_room:
graph[current_room] = {}
for room_exit in current_exits:
    # print(f"room_exit: {room_exit}")
    # print(f"current_exits: {current_exits}")
    graph[current_room][room_exit] = '?'

# PICK an unexplored exit (with "?") & SAVE as "chosen_exit":
chosen_exit = random.choice([room_exit for room_exit, room in graph[current_room].items()])
print(f"Chosen_exit: {chosen_exit}")

# ADD CHOSEN EXIT TO traversal_path & reversal_path: 
traversalPath.append(chosen_exit)
roomPath.append(current_room)
print(graph)
print(f"Room {current_room} exits: {player.currentRoom.getExits()}")
print(traversalPath)
print(roomPath)
print("================")

# SAVE current_room AS previous_room before moving:
previous_room = current_room

# Move to chosen_exit:
player.travel(chosen_exit)
print(f"Now in room: {player.currentRoom.id}")
print(f"Previous room: {previous_room}")


#################################################
######## TESTING STEPS FOR WHILE LOOP ###########
#################################################
...
# BUILD GRAPH for current_room (with all exits equal to '?'):
graph[current_room] = {}
for room_exit in current_exits:
    graph[current_room][room_exit] = '?'

# UPDATE GRAPHS -> PREVIOUS (using last room in "path") & CURRENT (using "current_room"):
graph[previous_room][chosen_exit] = current_room
graph[current_room][exit_inverse[chosen_exit]] = previous_room
...
# If current_room has unexplored exits then randomly select one to head there
if "?" in graph[current_room].values():
    # list of unexplored exits in current_room
    unexplored_exits = [room_exit for room_exit, room in graph[current_room].items() if room == "?"]
    print(f"Unexplored Exits: {unexplored_exits}")

    # picking one of the unexplored exits
    chosen_exit = random.choice(unexplored_exits)
    print(f"Chosen_exit: {chosen_exit}")

# # if no unexplored exits in current_room, BACKTRACK UNTIL A ROOM WITH UNEXPLORED EXIT IS FOUND
# else: 
#     print(f"reversalPath: {reversalPath}")
#     chosen_exit = exit_inverse[reversalPath[-1]]

#     # remove last exit in reversal path (to keep reverse path current):
#     reversalPath.pop()

# ADD CHOSEN EXIT TO traversal_path (to keep track of path taken and be able to return): 
traversalPath.append(chosen_exit)
roomPath.append(current_room)
...
# if no unexplored exits in current_room, BACKTRACK UNTIL A ROOM WITH UNEXPLORED EXIT IS FOUND
    ...
    for room_visited in reversed(roomPath):
        if "?" in graph[room_visited].values():
            with_unexplored = room_visited
            print(f"Room 'with_unexplored': {with_unexplored}")
    
    print(f"traversalPath: {traversalPath}")

    i = -1
    # traverse back until we find room that leads to room "with_unexplored" exit
    while with_unexplored not in graph[current_room].values():
        
        # SAVE previous room:
        previous_room = current_room

        # Backtrack one room at a time
        exit_direction = exit_inverse[traversalPath[i]]
        # print(f"Path Inversed--> {exit_direction}")
        player.travel(exit_direction)

        # SAVE current room
        current_room = player.currentRoom.id
        print(f"Now in room: {current_room}")
        print(f"Previous room--: {previous_room}")
        print(graph)
        print(f"Room {current_room} exits: {player.currentRoom.getExits()}")
        print(traversalPath)
        ...
        # Negatively increase counter
        i += -1
    
    # Confirm that we arrived at room that leads to "with_unexplored" room
    if with_unexplored in graph[current_room].values():
        
        # getting exit direction that leads to "with_unexplored" room
        # "[0]" is to extract string value from list
        exit_direction = [key for key, value in graph[current_room].items() if value == with_unexplored][0]
        print(f"What direction is it? -> {exit_direction}")

        # SAVE previous room:
        previous_room = current_room

        # MOVE to room to "with_unexplored" room
        player.travel(exit_direction)
...
#         # list of unexplored exits in current_room
#         unexplored_exits = [room_exit for room_exit, room in graph[current_room].items() if room == "?"]
#         print(f"FOUND Unexplored Exits!: {unexplored_exits}")
...
#     else:
#         print("NO UNEXPLORED EXITS FOUND!")
...
# ___ Loop Until Graph's Length is Same as Number of Rooms_____
...
#         # ADD CHOSEN EXIT TO traversal_path (to keep track of path taken and be able to return): 
#         traversalPath.append(chosen_exit)
#         reversalPath.append(chosen_exit)
...
#         previous_room = current_room
#         print(f"reversalPath: {reversalPath}")
#         if len(reversalPath) > 1:
#             player.travel(exit_inverse[reversalPath[-2]])
#         else:
#             player.travel(exit_inverse[reversalPath[-1]])
#         # remove last exit in reversal path (to keep reverse path current):
#         reversalPath.pop()
#         current_room = player.currentRoom.id
